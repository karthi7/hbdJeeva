...
const imagesLoaded = require('imagesloaded'); // Linear interpolation


const lerp = (a, b, n) => (1 - n) * a + n * b; // Gets the mouse position
...
},{"./gsap-core.js":"TNS6","./CSSPlugin.js":"bp4Z"}],"+9LM":[function(require,module,exports) {
"use strict";
...
},{"gsap":"TpQl","./utils":"8MgT"}],"oR66":[function(require,module,exports) {
...
},{"splitting":"Qdhu"}],"PEHS":[function(require,module,exports) {
"use strict";
...
class Slideshow {
  constructor(el) {
    ...
    this.slides = [];
    [...this.DOM.el.querySelectorAll('.slide')].forEach(slide => this.slides.push(new _slide.default(slide))); // Index of the current slide

    this.current = 0; // Total slides

    this.slidesTotal = this.slides.length; // positions (in percentages) for the images movement for both directions
  ...
  navigate(direction) {
    ...
    this.current = direction === 'next' ? this.current < this.slidesTotal - 1 ? ++this.current : 0 : this.current > 0 ? --this.current : this.slidesTotal - 1;
    ...
    }).addLabel('start', 0) // Animate current title out (stagger the characters)
    .to(currentSlide.DOM.chars, {
      y: direction === 'next' ? '100%' : '-100%',
      rotation: direction === 'next' ? 6 : -6,
      duration: 0.4,
      ease: 'power3.inOut',
      stagger: direction === 'next' ? -0.01 : 0.01
    }, 'start') // Animate current images out
    ...
    }, 'start').addLabel('upcoming', 0.1).add(() => {
      // Set up upcoming images and text default style:
      _gsap.gsap.set(nextSlide.DOM.imgs, {
        opacity: 0
      });

      _gsap.gsap.set(nextSlide.DOM.chars, {
        y: direction === 'next' ? '-100%' : '100%',
        rotation: direction === 'next' ? 6 : -6
      }); // Add "current" class
    ...
    }, 'upcoming') // Animate upcoming title in (stagger the characters)
    .to(nextSlide.DOM.chars, {
      ...
      ease: 'back',
      stagger: direction === 'next' ? -0.01 : 0.01
    }, 'upcoming+=0.1') // Animate upcoming images in
...
},{"splitting/dist/splitting.css":"oR66","splitting/dist/splitting-cells.css":"yIBK","gsap":"TpQl","../slide":"Drnq"}],"toeh":[function(require,module,exports) {
"use strict";
...
Promise.all([(0, _utils.preloadImages)('.slide__img'), (0, _utils.preloadFonts)('ldj8uhs')]).then(() => {
  // Remove loader (loading class)
  document.body.classList.remove('loading'); // Initialize custom cursor

  const cursor = new _cursor.default(document.querySelector('.cursor')); // Initialize the slideshow
...
},{"../utils":"8MgT","../cursor":"+9LM","./slideshow":"PEHS"}]},{},["toeh"], null)